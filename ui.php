...
<html lang="en" class="h-full">
 ...
 <body class="h-full bg-slate-100 overflow-hidden">
  <div id="app" class="h-full flex flex-col">
   ...
   <div class="flex-1 flex overflow-hidden">
    <!-- Sidebar Navigation -->
    <aside class="w-16 bg-white border-r border-slate-200 flex flex-col items-center py-4 gap-2 flex-shrink-0">
     <button onclick="switchView('pos')" id="nav-pos" class="nav-btn w-12 h-12 rounded-lg flex items-center justify-center bg-primary text-white" title="POS">
    ...
    <div id="view-pos" class="flex-1 flex overflow-hidden">
     ...
     <div class="flex-1 flex flex-col overflow-hidden p-4">
      ...
      <div class="flex items-center gap-3 mb-4 flex-shrink-0">
       ...
       </div><select id="category-filter" class="px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary"> <option value="all">All Categories</option> </select>
      </div><!-- Products Grid -->
      ...
      <div class="border-t border-slate-200 p-4 space-y-3 bg-slate-50">
       <div class="space-y-2 text-sm">
        <div class="flex justify-between">
         <span class="text-muted">Subtotal</span> <span class="font-medium" id="cart-subtotal">$0.00</span>
        </div>
        <div class="flex justify-between">
         <span class="text-muted">Tax (0%)</span> <span class="font-medium" id="cart-tax">$0.00</span>
        </div>
        <div class="flex justify-between">
         <span class="text-muted">Discount</span> <span class="font-medium text-accent" id="cart-discount">-$0.00</span>
        </div>
        <div class="flex justify-between pt-2 border-t border-slate-200">
         <span class="font-semibold text-slate-800">Total</span> <span class="font-bold text-lg text-primary" id="cart-total">$0.00</span>
        </div>
       </div><!-- Payment Section -->
       <div class="space-y-2">
        <div class="flex gap-2">
         <input type="number" id="paid-amount" placeholder="Amount Paid" class="flex-1 px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary"> <select id="payment-method" class="px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary"> <option value="cash">Cash</option> <option value="card">Card</option> <option value="mobile">Mobile</option> </select>
        </div>
        <div class="flex justify-between text-sm">
         <span class="text-muted">Change</span> <span class="font-medium text-accent" id="change-amount">$0.00</span>
        </div>
       </div><!-- Action Buttons -->
       <div class="grid grid-cols-2 gap-2 pt-2">
        <button onclick="holdSale()" class="px-4 py-2.5 bg-slate-200 text-slate-700 rounded-lg font-medium text-sm hover:bg-slate-300 transition-colors"> Hold Sale </button> <button onclick="completeSale()" id="complete-sale-btn" class="px-4 py-2.5 bg-primary text-white rounded-lg font-medium text-sm hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" disabled> Complete Sale </button>
       </div>
      </div>
     </div>
    </div><!-- Products Management View -->
    <div id="view-products" class="flex-1 p-6 overflow-y-auto hidden">
     <div class="max-w-6xl mx-auto">
      <div class="flex items-center justify-between mb-6">
       ...
       </div><button onclick="openProductModal()" class="px-4 py-2 bg-primary text-white rounded-lg font-medium text-sm hover:bg-blue-700 flex items-center gap-2">
    ...
    <div id="view-categories" class="flex-1 p-6 overflow-y-auto hidden">
     <div class="max-w-4xl mx-auto">
      <div class="flex items-center justify-between mb-6">
       ...
       </div><button onclick="openCategoryModal()" class="px-4 py-2 bg-primary text-white rounded-lg font-medium text-sm hover:bg-blue-700 flex items-center gap-2">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewbox="0 0 24 24">
         <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
    ...
    <div id="view-sales" class="flex-1 p-6 overflow-y-auto hidden">
     <div class="max-w-6xl mx-auto">
      <div class="flex items-center justify-between mb-6">
       ...
       <div class="flex items-center gap-3">
        <input type="date" id="sales-date-filter" class="px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary">
       </div>
      </div>
      <div class="bg-white rounded-lg border border-slate-200 overflow-hidden">
       <table class="w-full">
    ...
    <div id="view-settings" class="flex-1 p-6 overflow-y-auto hidden">
     <div class="max-w-2xl mx-auto">
      ...
      <div class="bg-white rounded-lg border border-slate-200 divide-y divide-slate-100">
       <div class="p-4">
        <label class="block text-sm font-medium text-slate-700 mb-2">Store Name</label> <input type="text" id="setting-store-name" placeholder="Enter store name" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary">
       </div>
       <div class="p-4">
        <label class="block text-sm font-medium text-slate-700 mb-2">Currency Symbol</label> <input type="text" id="setting-currency" value="$" maxlength="3" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary">
       </div>
       <div class="p-4">
        <label class="block text-sm font-medium text-slate-700 mb-2">Tax Rate (%)</label> <input type="number" id="setting-tax-rate" value="0" min="0" max="100" step="0.1" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary">
       </div>
       <div class="p-4">
        <label class="block text-sm font-medium text-slate-700 mb-2">Low Stock Threshold</label> <input type="number" id="setting-low-stock" value="10" min="1" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary">
       </div>
       <div class="p-4">
        <button onclick="saveSettings()" class="px-4 py-2 bg-primary text-white rounded-lg font-medium text-sm hover:bg-blue-700"> Save Settings </button>
       </div>
      </div>
     </div>
    </div>
   ...
   <div id="product-modal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-lg w-full max-w-md mx-4 overflow-hidden">
     <div class="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
      <h3 class="font-semibold text-slate-800" id="product-modal-title">Add Product</h3><button onclick="closeProductModal()" class="text-slate-400 hover:text-slate-600">
       <svg class="w-5 h-5" fill="none" stroke="currentColor" viewbox="0 0 24 24">
     ...
     <div class="p-4 space-y-4">
      ...
      <div>
       <label class="block text-sm font-medium text-slate-700 mb-1">Product Name</label> <input type="text" id="product-name" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary" placeholder="Enter product name">
      </div>
      <div>
       <label class="block text-sm font-medium text-slate-700 mb-1">Barcode</label> <input type="text" id="product-barcode" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary" placeholder="Enter barcode">
      </div>
      <div>
       <label class="block text-sm font-medium text-slate-700 mb-1">Category</label> <select id="product-category" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary"> <!-- Categories will be inserted here --> </select>
      </div>
      <div class="grid grid-cols-2 gap-4">
       <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Cost Price</label> <input type="number" id="product-cost" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary" placeholder="0.00" step="0.01" min="0">
       </div>
       <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Selling Price</label> <input type="number" id="product-price" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary" placeholder="0.00" step="0.01" min="0">
       </div>
      </div>
      <div>
       <label class="block text-sm font-medium text-slate-700 mb-1">Stock Quantity</label> <input type="number" id="product-stock" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary" placeholder="0" min="0">
      </div>
     </div>
     <div class="px-4 py-3 bg-slate-50 flex justify-end gap-2">
      <button onclick="closeProductModal()" class="px-4 py-2 text-slate-600 hover:bg-slate-200 rounded-lg font-medium text-sm">Cancel</button> <button onclick="saveProduct()" class="px-4 py-2 bg-primary text-white rounded-lg font-medium text-sm hover:bg-blue-700">Save Product</button>
     </div>
    </div>
   </div><!-- Category Modal -->
   <div id="category-modal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-lg w-full max-w-sm mx-4 overflow-hidden">
     <div class="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
      <h3 class="font-semibold text-slate-800" id="category-modal-title">Add Category</h3><button onclick="closeCategoryModal()" class="text-slate-400 hover:text-slate-600">
       <svg class="w-5 h-5" fill="none" stroke="currentColor" viewbox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
       </svg></button>
     </div>
     <div class="p-4">
      <input type="hidden" id="edit-category-id"> <label class="block text-sm font-medium text-slate-700 mb-1">Category Name</label> <input type="text" id="category-name" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-primary" placeholder="Enter category name">
     </div>
     <div class="px-4 py-3 bg-slate-50 flex justify-end gap-2">
      <button onclick="closeCategoryModal()" class="px-4 py-2 text-slate-600 hover:bg-slate-200 rounded-lg font-medium text-sm">Cancel</button> <button onclick="saveCategory()" class="px-4 py-2 bg-primary text-white rounded-lg font-medium text-sm hover:bg-blue-700">Save Category</button>
     </div>
    </div>
   </div><!-- Receipt Modal -->
   <div id="receipt-modal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden">
    <div class="bg-white rounded-lg w-full max-w-sm mx-4 overflow-hidden">
     <div class="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
      <h3 class="font-semibold text-slate-800">Receipt</h3><button onclick="closeReceiptModal()" class="text-slate-400 hover:text-slate-600">
       <svg class="w-5 h-5" fill="none" stroke="currentColor" viewbox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
       </svg></button>
     </div>
     <div id="receipt-content" class="p-4 text-sm font-mono">
      <!-- Receipt content will be inserted here -->
     </div>
     <div class="px-4 py-3 bg-slate-50 flex justify-end gap-2">
      <button onclick="closeReceiptModal()" class="px-4 py-2 text-slate-600 hover:bg-slate-200 rounded-lg font-medium text-sm">Close</button> <button onclick="printReceipt()" class="px-4 py-2 bg-primary text-white rounded-lg font-medium text-sm hover:bg-blue-700">Print</button>
     </div>
    </div>
   </div><!-- Toast Notification -->
   <div id="toast" class="fixed bottom-4 right-4 px-4 py-3 bg-slate-800 text-white rounded-lg shadow-lg transform translate-y-20 opacity-0 transition-all duration-300 z-50">
  ...
  <script>
    ...
    const products = [
      { id: 1, name: 'Coca Cola 500ml', barcode: '8901234567890', categoryId: 1, cost: 0.80, price: 1.50, stock: 50 },
      { id: 2, name: 'Pepsi 500ml', barcode: '8901234567891', categoryId: 1, cost: 0.75, price: 1.50, stock: 45 },
      { id: 3, name: 'Sprite 500ml', barcode: '8901234567892', categoryId: 1, cost: 0.80, price: 1.50, stock: 40 },
      { id: 4, name: 'Lays Classic 150g', barcode: '8902345678901', categoryId: 2, cost: 1.20, price: 2.50, stock: 30 },
      { id: 5, name: 'Doritos Nacho 180g', barcode: '8902345678902', categoryId: 2, cost: 1.50, price: 3.00, stock: 25 },
      { id: 6, name: 'Pringles Original', barcode: '8902345678903', categoryId: 2, cost: 2.00, price: 3.50, stock: 20 },
      { id: 7, name: 'Fresh Milk 1L', barcode: '8903456789012', categoryId: 3, cost: 1.00, price: 1.80, stock: 60 },
      { id: 8, name: 'Greek Yogurt 500g', barcode: '8903456789013', categoryId: 3, cost: 2.50, price: 4.00, stock: 35 },
      { id: 9, name: 'Cheddar Cheese 250g', barcode: '8903456789014', categoryId: 3, cost: 3.00, price: 5.50, stock: 25 },
      { id: 10, name: 'White Bread Loaf', barcode: '8904567890123', categoryId: 4, cost: 1.00, price: 2.00, stock: 40 },
      { id: 11, name: 'Croissant 4-pack', barcode: '8904567890124', categoryId: 4, cost: 2.00, price: 3.50, stock: 20 },
      { id: 12, name: 'Chocolate Muffin', barcode: '8904567890125', categoryId: 4, cost: 1.20, price: 2.50, stock: 30 },
      { id: 13, name: 'Ice Cream Vanilla 1L', barcode: '8905678901234', categoryId: 5, cost: 3.00, price: 5.00, stock: 15 },
      { id: 14, name: 'Frozen Pizza', barcode: '8905678901235', categoryId: 5, cost: 4.00, price: 7.00, stock: 18 },
      { id: 15, name: 'Frozen Fries 1kg', barcode: '8905678901236', categoryId: 5, cost: 2.50, price: 4.50, stock: 22 },
      { id: 16, name: 'Rice 5kg', barcode: '8906789012345', categoryId: 6, cost: 5.00, price: 8.00, stock: 50 },
      { id: 17, name: 'Pasta 500g', barcode: '8906789012346', categoryId: 6, cost: 1.00, price: 2.00, stock: 60 },
      { id: 18, name: 'Olive Oil 500ml', barcode: '8906789012347', categoryId: 6, cost: 4.00, price: 7.00, stock: 25 }
    ];
    ...
    function switchView(view) {
      const views = ['pos', 'products', 'categories', 'sales', 'reports', 'settings'];
      views.forEach(v => {
 ...
 <script>(function(){function c(){var b=a.contentDocument||a.contentWindow.document;if(b){var d=b.createElement('script');d.innerHTML="window.__CF$cv$params={r:'9d8bc05d425720c7',t:'MTc3MjkwOTA2Ni4wMDAwMDA='};var a=document.createElement('script');a.nonce='';a.src='/cdn-cgi/challenge-platform/scripts/jsd/main.js';document.getElementsByTagName('head')[0].appendChild(a);";b.getElementsByTagName('head')[0].appendChild(d)}}if(document.body){var a=document.createElement('iframe');a.height=1;a.width=1;a.style.position='absolute';a.style.top=0;a.style.left=0;a.style.border='none';a.style.visibility='hidden';document.body.appendChild(a);if('loading'!==document.readyState)c();else if(window.addEventListener)document.addEventListener('DOMContentLoaded',c);else{var e=document.onreadystatechange||function(){};document.onreadystatechange=function(b){e(b);'loading'!==document.readyState&&(document.onreadystatechange=e,c())}}}})();</script></body>
</html>